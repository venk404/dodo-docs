...
description: 'Deploy DodoPayments webhooks to Supabase Edge Functions with built-in database'
icon: 'bolt'
---
...
1. **Receives webhook request** - Handles POST from DodoPayments with webhook headers
2. **Verifies signature** - Uses standardwebhooks library for HMAC-SHA256 verification
3. **Checks idempotency** - Looks up webhook ID to prevent duplicate processing
4. **Logs the event** - Stores raw webhook data in `webhook_events` table
5. **Processes updates** - Creates or updates customers and subscriptions via Supabase client
6. **Handles errors** - Logs failures and marks event for retry
...
Three tables created by `schema.sql`:
