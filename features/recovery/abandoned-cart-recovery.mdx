...
A **5% recovery fee** is charged on successfully recovered revenue (only when at least one recovery email was sent).
...
Below is an example of what a recovery email looks like when received by a customer.

<Frame caption="Example ACR recovery email as seen by the customer">
<img src="/images/recovery/acr-email-example.png" alt="Example abandoned cart recovery email showing the store name, product details, recovery link button, and unsubscribe option" />
</Frame>
...
ACR charges a **5% recovery fee** on successfully recovered revenue. The fee is calculated on the total payment amount including taxes and other fees.

| Condition | Fee Charged? |
|-----------|-------------|
| Customer pays via recovery link after receiving at least 1 email | Yes (5%) |
| Customer completes purchase independently before any email is sent | No |
| Customer opts out of recovery emails | No |

<Info>
Recovery fees appear as `abandoned_cart_recovery_fee` entries in your [Balance Ledger](/api-reference/balance-ledger/list-ledger-entries).
</Info>
...
<Frame caption="ACR analytics showing recovery rates and revenue metrics">
<img src="/images/recovery/acr-analytics.png" alt="ACR analytics dashboard with charts showing abandoned checkout counts, recovery rates, recovered revenue, and per-email performance" />
</Frame>
...
- **Test in test mode**: ACR settings are configured separately for test and live modes. Test your email sequences before going live.
